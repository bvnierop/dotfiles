...
Finally, updating with ~package.el~ is a hassle. I'm hoping it's easier with ~straight.el~.
...
(defun isearch-kill-last-word ()
  ...
  (with-isearch-suspended
   (with-temp-buffer
     ...
     (setq isearch-new-string (buffer-string)
           isearch-new-message (buffer-string)))))
...
(defun fsharp/fsharp-mode-config ()
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ruby/enh-ruby-mode-config ()
    (enable-minor-modes 'dtrt-indent-mode :for 'enh-ruby-mode)
...
*** TODO Docker
#+BEGIN_SRC emacs-lisp
(bvn/use-packages dockerfile-mode)
